...
//
// Later it will call an init() function that takes configuration options stored as an array of strings

public class CoreCipherClassLibrary {
	public static Cipher instantiateFromString(String cipherName, String args[]) throws Exception {
		try { // uses the static forName function of Class to load the named class, and then calls that classes newInstance() method to instantiate it
			Cipher myCipher = (Cipher)Class.forName("cipher."+cipherName+"Cipher").newInstance();
			myCipher.init(args);
			return myCipher;
		}
